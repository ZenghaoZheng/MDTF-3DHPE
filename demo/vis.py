...
from demo.LOADMODEL import loadweightmodel
...
def calculate_angles(polar_angle, x):
    eps = 1e-7
    b, f, j, c = x.shape
    polar_angle = torch.argmax(polar_angle, dim=-1)  # b,f,17 , 有6类
...
def get_pose3D(video_path, output_dir):
    ...
    for idx, clip in enumerate(clips):
        ...
        polarangle = model1(input_2D)
        input_2D = calculate_angles(polarangle, input_2D)
        output_3D = model2(input_2D)
